...
public class MultilayerPerceptron 
  ...
  protected class NeuralEnd 
	...
	public String getRevision() {
      return RevisionUtils.extract("$Revision: 5613 $");
    }
  }
  ...
  private class NodePanel 
	...
	public void mousePressed(MouseEvent e) {
		...
		if (m_neuralNodes[noa].onUnit(g, x, y, w, h)) {
	      ...
	      tmp.addElement(temp);
	      addNode(temp);
	      selection(tmp, (e.getModifiers() & InputEvent.CTRL_MASK) == InputEvent.CTRL_MASK
			, true);
	    }
	    else {
	      //then right click
		...
		if (m_neuralNodes[noa].onUnit(g, x, y, w, h)) {
	      ...
	      selection(null, (e.getModifiers() & InputEvent.CTRL_MASK) == InputEvent.CTRL_MASK
			, false);
	    }
	  }
	});
...
public void buildClassifier(Instances i) throws Exception {
	...
	public void windowClosing(WindowEvent e) {
	    ...
	    if (well == 0) {
	      m_win.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
	      m_accepted = true;
	      blocker(false);
	    }
	    else {
	      m_win.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
	    }
	    m_stopIt = k;
	  }
	});
