...
public class BVDecomposeSegCVSub
    ...
    public void decompose() throws Exception {
        ...
        if ( m_TrainSize == -1 ){ // default value
            m_TrainSize = (int) Math.floor( data.numInstances() / 2.0 );
        ...
        if ( m_P == -1 ){ // default value
            m_P = m_TrainSize / ( (double)data.numInstances() - 1 );
        ...
        tps = (int) Math.ceil( (m_TrainSize / m_P) + 1 );
        ...
        m_KWBias /= (2.0 * data.numInstances());
        m_KWVariance /= (2.0 * data.numInstances());
        m_KWSigma /= (2.0 * data.numInstances());
