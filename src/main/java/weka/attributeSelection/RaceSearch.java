...
public double [][] rankedAttributes() throws Exception {
  ...
  private int [] schemataRace(Instances data, Random random) throws Exception {
    ...
    for (int i=0;i<m_numAttribs;i++) {
      if (i != m_classIndex) {
        parallelRaces[count][0] = base.clone();
        parallelRaces[count][1] = base.clone();
    ...
    raceSet: while (numRaces > 0) {
      ...
      if (numRaces > 0 && won) {
        ...
        count=0;
        for (int i=0;i<numRaces;i++) {
          parallelRaces[i][0] = base.clone();
          parallelRaces[i][1] = base.clone();
  ...
  private int [] rankRace(Instances data, Random random) throws Exception {
    ...
    // set up the race
    raceSets[0] = baseSet.clone();
    for (int i=0;i<m_Ranking.length;i++) {
      raceSets[i+1] = raceSets[i].clone();
    ...
    bestSet = raceSets[(int)winnerInfo[0]].clone();
  ...
  private int [] hillclimbRace(Instances data, Random random) throws Exception {
    ...
    baseSet = raceSets[(int)winnerInfo[0]].clone();
    if (m_rankingRequested) {
